...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    if (user.emailVerified === false) {
      userStatus.innerHTML = `Goto setttings and verify yourself.`;
    } else {
...
const uploadFile = async () => {
  ...
  if (!file) {
    toastr.error("Please select a picture first.");
    return;
  }

  // File size validation
  if (file.size > 1572864) {
    toastr.error("Picture must be less than 1.5Mb.");
    return;
  }
...
const privacyProfile = async () => {
  ...
  const docSnap = await getDoc(docRef);
  if (docSnap.exists()) {
    let { password, email } = docSnap.data();
...
const editPassword = async () => {
  ...
  if (newPassword === "") {
    ...
    toastr.error(
      "Password should be only digits or numbers and minmimum Eight characters."
    );
  } else {
